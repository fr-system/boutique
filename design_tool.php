        ...
        <div class="input-label flex-display <?php echo  $add_class . ($column["widget"] != "textarea" && $column["widget"] != "table"  ? 'align-center' :'stretch') ?> ">
            ...
            if(isset($column["field_name"])) {
                if ($column["widget"] == "table") {
                    ...
                    if (isset($single->id)) {
                        ...
                        if(!isset($column["target_table"])) {
                            ...
                            foreach ($single_sub_data as $sub_row) {//קבץ את שורות ההזמנה לפי product_id
                                $grouped_list[$sub_row->$sub_table_id][] = $sub_row;
                            }
                        }
                    }
                    ...
                    $list = isset($single->$field_name) ? $single->$field_name : "";
                }
...
function render_row($external_item, $sub_row, $options ){
    ...
    $sub_columns = BOUTIQUE_TABLES[$options["sub_table"]]["columns"];
...
{//space-between
    ...
    <div class="archive-actions flex-display start  margin-bottom-20">
        <div class="flex-display align-center space-between">
            ...
            <h1 class="page-title font-30 bold padding-left-40"><?php echo $title.(isset($attr["add_text"]) && !empty($attr["add_text"])?$attr["add_text"]:'')  ?></h1>
        </div>
        <div class="flex-display align-center  space-between">
            <?php if(!$view_only){
            ...
            <?php }?>
            <svg class="margin-after-10 has-tooltip" xmlns="http://www.w3.org/2000/svg" onclick="viewFilterByArea()" data-tooltip="סנן לפי" width="44" height="44" viewBox="0 0 44 44" fill="none">
            ...
            <div class=" flex-display align-center filter-by-area hidden">
                    <?php
                    $html = "";
                    foreach ($columns as $column){
                    ...
                    <input type="text" class="filter-value hidden filter-value">
                ...
                <script type="text/javascript">jQuery(document).ready(function($) {jQuery("select.filter-by").trigger("change");})</script>
            </div>
