...
function get_tr_data($table_name, $data, $key,$attr){
    ...
    if(!isset($page_info["update_remove"]) || $page_info["update_remove"] == true) {
        ...
        $html .= '<td><a   class="has-tooltip" data-tooltip="הדפסת ' . $page_info['single'] . '" href="'.get_bloginfo('stylesheet_directory').'/lib/export_pdf.php?file=pdf&export=single&subject='.$table_name.'&id=' . $row->id . '" target="_blank" >
